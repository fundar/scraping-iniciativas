	...
	if(isset($explode[1])) {
		...
		if(isset($explode[1])) {
			...
			$nombre  = $explode[0];
                       ...
                       $explode = explode("<td class='tddatosazul'>Senador Suplente:<br><b>", $resultado);
		if(isset($explode[1])) {
			$explode = explode('<br>', $explode[1]);
			$nombre  = $explode[0];
		}
                   else {
		$explode = explode("<td class='tddatosazul'>Senadora Suplente:<br><b>", $resultado);
		if(isset($explode[1])) {
			$explode = explode('<br>', $explode[1]);
			$nombre  = $explode[0];
		}
...
//Ubicacion
 ...
 //comisiones
  $explode = explode("tdcriterio'>Comisi&oacute;n</td>", $resultado);
	    if(isset($explode[1])) {
        $explode = explode("<td class='tddatosazul'>", $explode[1]);
		$explode = explode("</td></tr>", $explode[4]);
...
echo "http://sil.gobernacion.gob.mx/Archivos/Fotos/".$id.".jpg"."|" . $nombre . "|" . $partido . "|" . $email . "|" . $telefono .  "|" . $nacimiento . "|" . $entidad ."|" . $ciudad . "|" . $eleccion .  "|" . $zona . "|" . $protesta . "|".  $ubicacion . "|".   $suplente . "|".    $grado . "|".    $carrera . "|".    $xp . "|". $comisiones . "|".  $suplentede . "|".  "<br/>";
